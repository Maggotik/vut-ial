...
void queueInit (tQueue* q) {
	//kontrola či parameter nieje NULL
	if(q==NULL){
		queueError(QERR_INIT);
		return;
	}
	//cyklus nastavý všetky hodnoty v poli na '*'
...
int nextIndex (int index) {
	return (index + 1) % QUEUE_SIZE; //vráti index nasledujúceho prvku v poli
/*
...
int queueFull (const tQueue* q) {
	//kontrola či je fronta plná
	return(((q->f_index - 1) == q->b_index) || ((q->f_index == 0) && (q->b_index == QUEUE_SIZE - 1)));
/*
...
void queueFront (const tQueue* q, char* c) {
	//kontrola či nieje fronta prázdna
	if(queueEmpty(q)){
