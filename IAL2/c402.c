...
void BTInsert (tBTNodePtr *RootPtr, int Content) {
	...
	tBTNodePtr add_node = (tBTNodePtr)malloc(sizeof(struct tBTNode));
	if(!add_node)
	{
		return;
	}
	add_node->Cont = Content;
	add_node->LPtr = NULL;
	add_node->RPtr = NULL;
	if(insert_node->Cont > Content)
	{
		insert_node->LPtr = add_node;
		return;
	}
	if(insert_node->Cont < Content)
	{
		insert_node->RPtr = add_node;
		return;
	}
/*   --------
...
void BTPostorder (tBTNodePtr RootPtr)	{
	...
	SInitP(&tmpP);
	SInitB(&tmpB);
	Leftmost_Postorder(RootPtr, &tmpP, &tmpB);

	while(!SEmptyP(&tmpP))
	{
		RootPtr = STopPopP(&tmpP);
		zlava = STopPopB(&tmpB);
		if(zlava)
		{
			SPushB(&tmpB, false);
			SPushP(&tmpP, RootPtr);
			Leftmost_Postorder(RootPtr->RPtr, &tmpP, &tmpB);
		}
		else
		{
			BTWorkOut(RootPtr);
